...
const headerCartCounter = document.getElementById('cartspan')
...
const storageOrders = localStorage.getItem("phones")
if (storageOrders) { // если данные есть
    // преобразуем данные в массив ordersList
    ordersList = JSON.parse(storageOrders)
    const oldModels = [] // массив заказов моделей, отсутствующих в текущем файле PHONES
    ...
    if (ordersList.length > 0) {
        ...
        for (let i = 0; i < ordersList.length; i++) {
            // проверяем, не удалена ли заказанная модель из PHONES
            const prodIndex = PHONES.findIndex( phone => phone.brand + ' ' + phone.model === ordersList[i].name )
            if (prodIndex >= 0) {
                orderedSmartphoneCounter += ordersList[i].number // если не удалена - вносим в заказы
            } else {
                oldModels.push(ordersList[i].name) // если не нашли в PHONES - добавляем в список на удаление из памяти
            }
        }
    }
    ...
    if (oldModels.length > 0) { // если есть модели, которых больше нет в PHONES
        oldModels.forEach( name => {
            // фильтруем заказы
            ordersList = ordersList.filter( phone => phone.name !== name)
        })
        localStorage.setItem("phones", JSON.stringify(ordersList)) // обновляем хранилище
    }
}
...
function addPhoneToCatalog( phone ) {
    ...
    const description = `
        <div><span>встроенная память:</span> ${phone.ROM} <span>Гб</span></div>
        <div><span>оперативная память:</span> ${phone.RAM} <span>Гб</span></div>
        <div><span>Батарея:</span> ${phone.battery} <span>мА/час</span></div>
        <div><span>Вес:</span> ${phone.weight} <span>г</span></div>
        <div><span>Камера:</span> ${phone.camera} <span>МП</span></div>
        <div><span>Экран:</span> ${phone.screen} <span>"</span></div>     
        `
    ...
    descriptionDiv.append(orderContainer)
...
function addToCartClicked( event ) {
    ...
    const descriptionDiv = orderButtonContainer.parentElement.parentElement // order-container -> phone-bg
    const h3 = descriptionDiv.querySelector('h3')
...
function removeFromCartClicked( event ) {
    const button = event.target
    const orderButtonContainer = button.parentElement
    const phoneCard = orderButtonContainer.parentElement.parentElement.parentElement
    ...
    if (pageName === 'cart' && numberOfOrdered === 0) {
        ...
        if (orderedSmartphoneCounter === 0) {
            const info = `<h4 style="text-align: center">У вас нет заказов. Перейдите <a href="./index.html">в каталог</a> для выбора смартфонов.</h4>`
